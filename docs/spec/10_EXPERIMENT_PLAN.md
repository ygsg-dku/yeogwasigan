...
실험은 탐지·마스킹·라우팅 자체의 신규성을 주장하지 않는다. 검증 질문은 우선순위가 다른 두 층으로 나뉜다. [최종 포지셔닝](../research/FINAL_DIFFERENTIATION.md)을 따른다.

**주 질문 — 반출 전 검토 흐름**

> Q1. 업무마다 정해둔 목록에 있는 것만 보내도 AI가 원인을 제대로 찾는가? (같은 로그를 다 보낸 경우와 목록대로만 보낸 경우로 나눠, 같은 모델·질문·설정에서 비교)
>
> Q2. 검토·승인한 패키지만 등록 Destination에 도달하고, 승인 뒤 내용·목적지·정책 변경과 Secret 잔존, 미등록 T0, 직접 우회가 차단되는가? (수신 로그·시험)

Q1이 실패하면 목록이 부족하다는 뜻이다. 그 결과를 그대로 보고하고, 담기가 이 업무에 맞지 않는다는 결론도 결론으로 받아들인다.

**보강 질문 — 기회가 될 때만**

> Q3. 금융 IT 운영자와 보안 검토자가 별도 AI로 장애 로그를 보내기 전 사건별 사전 검토를 실제로 필요로 하는가? (현업 인터뷰)

Q3은 섭외가 불확실하므로 2차 발표의 계획에 넣지 않았다. 첫 목록은 공개된 장애 보고서와 합성 로그를 근거로 정하고, 인터뷰 기회가 생기면 목록을 보강한다. 질문지는 [2차 발표 대본](../presentations/여과시간_2차발표_선행기술조사_5분_대본.md)의 부록에 있다. Q3 없이도 Q1이 목록의 부족을 일부 드러낸다.
